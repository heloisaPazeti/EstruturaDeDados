...
Node *Node_Create(TypeData val)
{
    ...
    strncpy(node -> val.tag, val.tag, tagSize);
    ...
    if(L != NULL)
    {
        ...
        else
        {
            ...
            while(current != NULL)
            {
                ...
                if(strncmp(current -> val.tag, val.tag, tagSize) > 0)
                {
...
void List_Remove(List *L, TypeData val)
{
    if(!List_IsEmpty(L))
    {
        ...
        if(strncmp(L -> begin -> val.tag, val.tag, tagSize) == 0)
        {
            L -> begin = p -> next;
        ...
        else
        {
            ...
            while(p != NULL)
            {
                ...
                if(strncmp(p -> val.tag, val.tag, tagSize) == 0)
                {
                    p -> prev -> next = p -> next;
                    if(L -> end == p)
                    {
...
int List_Search(List *L, char tag[10])
{
    ...
    while(current != NULL)
    {
        if(strncmp(tag, current -> val.tag, 8) == 0)
            return pos;
